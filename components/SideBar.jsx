...
import ChatLabel from './ChatLabel'
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? "p-4 w-64" : "w-0 md:w-20 max-md:overflow-hidden"}`}>
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
            <Image src={expand ? assets.logo_text : assets.logo_icon} alt='deepseek-logo' className={expand ? "w-36" : "w-10"}/>

            <div onClick={()=>{expand ? setExpand(false): setExpand(true)}}  
             className='group relative items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                ...
                <div className={`absolute w-max ${expand ? "left-1/2 -translate-1/2 top-12" : "-top-12 left-0"} opacity-0 group-hover:opacity-100 bg-black transition text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
        ...
        <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" :"group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            ...
            <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none'>
                New chat
                <div className='w-3 h-3 absolute bg-black roate-45 left-4 -bottom-1.5'>
                </div>
            </div>
    ...
    <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            <Image src={expand ? assets.phone_icon : assets.phone_icon_dull} className={expand ? "w-5" : "w-6 mx-auto"} alt='qr-code'/>
           ...
           {expand && <><span> Get App</span><Image src={assets.new_icon} alt='new-icon'/></>}

        </div>

        <div className={`flex items-center ${expand ? "hover:bg-white/10 rounded-lg" : "justify-center w-full"} gap-3 text-white/60 text-sm p-2 mt-2 cursor-pointer`} onClick={user ? null : openSignIn}>
          {
            user ? <UserButton/> : <Image src={assets.profile_icon} alt='profile-icon' className='w-7'/>
